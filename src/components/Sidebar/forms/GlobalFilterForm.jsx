...
*/
export default function GlobalFilterForm(props) {
    const [isDate, setIsDate] = useState(false);
    const [isBool, setIsBool] = useState(false);
    const [isEditMode, setIsEditMode] = useState(false);

    const { globalFilters, headers } = props;
    ...
    const onSubmit = (e) => {
        ...
        const id = globalFilters.length ? Math.max(...globalFilters.map(({ id }) => id)) + 1 : 0;
        ...
        if (isEditMode !== false) {
            props.setGlobalFilters(globalFilters.map((el) => el.id !== isEditMode.id ? el : filter));
            setIsEditMode(false);
        } else {
            props.setGlobalFilters([...globalFilters, filter]);
        }
    ...
    const onRemove = (index) => (e) => {
        e.preventDefault();
        setIsEditMode(false);
        props.setGlobalFilters(globalFilters.filter((a, i) => a.id !== index));
    };
    ...
    const onToggle = (id) => (e) => {
        props.setGlobalFilters(globalFilters.map((a) => a.id !== id ? a : { ...a, active: !a.active }));
    };
